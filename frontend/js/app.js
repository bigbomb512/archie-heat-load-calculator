...
function showCalculatorInputs(inputSet = {}, context = {}, overrides = {}){
  ...
  const unavailableDefaults = (inputSet.research_defaults_unavailable || []).map(row => `${row.record_id}: ${row.reason || "not eligible"}`).join(" · ");
